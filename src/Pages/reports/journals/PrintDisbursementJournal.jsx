...
export function PrintDisbursementJournal () {
	...
	const downloadPDF = () => {
		...
		html2canvas(input).then((canvas) => {
			...
			pdf.save(`CashDisbursementJournal_${startDate}-${endDate}.pdf`);
		});
	};
	...
	const downloadExcel = () => {
		...
		worksheet['A1'] = { t: 's', v: 'Cash Disbursement Journal', s: {
		...
		worksheet['!merges'] = worksheet['!merges'].concat([{ s: { r: 1, c: 0 }, e: { r: 1, c: 3 } }]);
		
		worksheet['A3'] = { t: 's', v: `Generated on: ${formatDate(new Date())}`, s: {
			font: { sz: 10 },
			alignment: { horizontal: 'center' }
		} };
		worksheet['!merges'] = worksheet['!merges'].concat([{ s: { r: 2, c: 0 }, e: { r: 2, c: 3 } }]);
		
		// Add data starting from row 5
		const data = reportData.map(item => ({
			Date: formatDate(item.date),
			Agent: item.payeeName?.registeredName,
		...
		XLSX.utils.book_append_sheet(workbook, worksheet, "Disbursement Journal");
		
		// Save file
		XLSX.writeFile(workbook, `CashDisbursementJournal_${startDate}-${endDate}.xlsx`);
	};
	...
	return (
		<>
			...
			<div className="print-content max-w-4xl mx-auto px-6 py-8">
				<h1 className="text-2xl font-bold mb-1 text-center">
					Cash Disbursement Journal
				</h1>
				<h2 className="text-base text-gray-600 mb-2 text-center">
