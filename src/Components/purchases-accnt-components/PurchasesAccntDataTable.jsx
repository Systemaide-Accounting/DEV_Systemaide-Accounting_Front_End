...
import { deletePurchasesAccountTransaction, getAllDeletedPurchasesAccountTransactions, getAllPurchasesAccountTransactions, restorePurchasesAccountTransaction } from "../../services/systemaideService";
...
export function PurchasesAccntDataTable({ type }) {
  ...
  const fetchAllTransactions = async () => {
    try {
      const response = type === "deleted" ? await getAllDeletedPurchasesAccountTransactions() : await getAllPurchasesAccountTransactions();
      if (response?.success) {
    ...
    const handleRestoreTransaction = async (transactionId) => {
      await swal2
        ...
        .then(async (result) => {
          if (result.isConfirmed) {
            try {
              const response = await restorePurchasesAccountTransaction(
                transactionId
              );
              if (response?.success) {
                await fetchAllTransactions();
      ...
      <div className="bg-white p-4 dark:bg-gray-800 shadow-sm rounded-lg dark:border-gray-700">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-4">
          <h2 className="text-xl font-semibold">
            {type === "deleted" ? "Purchases On Account" : "Summary"}
          </h2>
