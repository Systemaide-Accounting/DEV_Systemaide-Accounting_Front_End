...
import { createSalesAccountTransaction, getAllLocations, getSalesAccountTransactionById, updateSalesAccountTransaction } from "../../services/systemaideService";
...
export function SalesOnAccountFormPage() {
    ...
    const handleSubmit = async (e) => {
      ...
      try {
            let response = null;
            if (transactionData) {
              response = await updateSalesAccountTransaction(
                transactionData?._id,
                JSON.stringify(updatedFormData)
              );
            } else {
              response = await createSalesAccountTransaction(
                JSON.stringify(updatedFormData)
              );
            }
            if (response?.success) {
    ...
    const fetchTransactionById = async (id) => {
        try {
            const response = await getSalesAccountTransactionById(id);
            if(response?.success) {
    ...
    return (
      <>
        ...
        <div className="w-full p-4 border rounded-lg bg-white mb-4 shadow">
          <h2 className="text-xl font-bold text-gray-900">Sales Journal</h2>
        </div>
